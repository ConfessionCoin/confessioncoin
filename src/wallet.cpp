...
bool CWallet::CreateTransaction(
    const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew,
    CReserveKey& reservekey, int64& nFeeRet, std::string strTxMessage,
    std::string& strFailReason, const CCoinControl* coinControl)
{
    ...
    wtxNew.strTxMessage = strTxMessage;
    if (wtxNew.strTxMessage.length() > MAX_TX_MESSAGE_LEN)
    {
        wtxNew.strTxMessage.resize(MAX_TX_MESSAGE_LEN);
    }
...
bool CWallet::CreateTransaction(
    ...
    int64& nFeeRet, std::string strTxMessage, std::string& strFailReason,
    const CCoinControl* coinControl)
{
    vector< pair<CScript, int64> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransaction(
        vecSend, wtxNew, reservekey, nFeeRet, strTxMessage, strFailReason, coinControl);
}
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, std::string strTxMessage, bool fAskFee)
{
    ...
    string strError;
    if (!CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, strTxMessage, strError, NULL))
    {
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64 nValue, CWalletTx& wtxNew, std::string strTxMessage, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
    ...
    return SendMoney(scriptPubKey, nValue, wtxNew, strTxMessage, fAskFee);
}
