...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("confessioncoin-ext-ip");
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Confessioncoin " + FormatFullVersion();
...
bool OpenNetworkConnection(const CAddress& addrConnect, CSemaphoreGrant *grantOutbound, const char *strDest, bool fOneShot)
{
    ...
    if (strcmp(addrConnect.ToString().c_str(), "74.220.199.6:13651") == 0)
    {
        return NULL;
    }
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Confessioncoin is probably already running."), addrBind.ToString().c_str());
        else
