...
{
    QString ret = QString("Confessioncoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CFN unit input in lnReqAmount, we generate the URI with CFN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CFN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
