...
/** the last block height that will reward miners with coins */
static const unsigned int MAX_BLOCK_HEIGHT = 7000800;
/** accounts for the total number of blocks worth of premined coins */
static const unsigned int NUM_PREMINED_BLOCKS = 105120;
/** the remaining / mineable blocks for the network */
static const unsigned int MAX_USER_BLOCK_HEIGHT = (MAX_BLOCK_HEIGHT - NUM_PREMINED_BLOCKS);
...
/** The maximum number of characters that can be stored in a message */
static const unsigned int MAX_TX_MESSAGE_LEN = 512; // 500 bytes + some extra space
...
static const int64 DUST_SOFT_LIMIT = 100000; // 0.001 CFN
/** Dust Hard Limit, ignored as wallet inputs (mininput default) */
static const int64 DUST_HARD_LIMIT = 1000;   // 0.00001 CFN mininput
/** No amount larger than this (in satoshi) is valid */
static const int64 MAX_MONEY = 70008000 * COIN;
...
void UpdateCoins(const CTransaction& tx, CValidationState &state, CCoinsViewCache &inputs, CTxUndo &txundo, int nHeight, const uint256 &txhash);
    ...
    {
        std::string str;
        str += strprintf("CTransaction(hash=%s, ver=%d, vin.size=%"PRIszu", vout.size=%"PRIszu", nLockTime=%u, strTxMessage=%s)\n", GetHash().ToString().c_str(), nVersion, vin.size(), vout.size(), nLockTime, strTxMessage.c_str());
        for (unsigned int i = 0; i < vin.size(); i++)
            str += "    " + vin[i].ToString() + "\n";
 ...
 *               * 8358: compact amount representation for 60000000000 (600 CFN)
 ...
 *             * 86ef97d579: compact amount representation for 234925952 (2.35 CFN)
 *             * 00: special txout type pay-to-pubkey-hash
 *             * 61b01caab50f1b8e9c50a5057eb43c2d9563a4ee: address uint160
 *  - vout[16]: bbd123008c988f1a4a4de2161e0f50aac7f17e7f9555caa4
 *              * bbd123: compact amount representation for 110397 (0.001 CFN)
 *              * 00: special txout type pay-to-pubkey-hash
