...
// Copyright (c) 2014 The ConfessionCoin developers
...
uint256 hashGenesisBlock("0x91cb4e0795a67f711149ee26b89d54172bc24b0155de6e7b42fe3bfebdc055f8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Confessioncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Confessioncoin Signed Message:\n";
...
{
    // Confessioncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(unsigned int nHeight, int64 nFees)
{
    int64 nSubsidy = 0;

    // block rewards end after about a year (block height 7,008,000)
    // Total coin supply: (7,008,000 * 10) = 70,080,000
    // user block height is the total reduced by the amount gained
    // from the premine
    if (nHeight < (MAX_USER_BLOCK_HEIGHT + 1))
    {
        // one time pre-mine on the first block
        if (nHeight == 1)
        {
            nSubsidy = 10 * COIN * NUM_PREMINED_BLOCKS;
        }
        else
        {
            nSubsidy = 10 * COIN;
...
static const int64 nTargetTimespan = 45; // ConfessionCoin: 45 second retargeting time
static const int64 nTargetSpacing = 45; // ConfessionCoin: 45 second block times
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Confessioncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("confessioncoin-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Confessioncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Confessioncoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4256fbffa2143463d662c2b802ea16ab74910a1400702623cc505524c3fd78f7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = (fTestNet ? "ConfessionCoin Testnet" :
                                    "Mondoweiss 25/Mar/2014 A Papal confession to the Palestinian people");
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1397773128;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = (unsigned int)2524238024;

        if (fTestNet)
        {
            block.nTime    = 1397773127;
            block.nNonce   = 2524755642;
        }

        //// debug print
        uint256 hash = block.GetHash();
        assert(block.hashMerkleRoot == uint256("0x361ea6a3249bc96bd23457657f157a5a626b371f97ac17a54ecb0e2fa853135d"));
...
unsigned char pchMessageStart[4] = { 0xf7, 0xbc, 0xb2, 0xd7 }; // Confessioncoin: decrease each by 4 from litecoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ConfessioncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ConfessioncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ConfessioncoinMiner(CWallet *pwallet)
{
    printf("ConfessioncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("confessioncoin-miner");
    ...
    try { loop {
        ...
        printf("Running ConfessioncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(), ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));
    ...
    {
        printf("ConfessioncoinMiner terminated\n");
        throw;
    }
}

void GenerateConfessionCoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ConfessioncoinMiner, pwallet));
}
